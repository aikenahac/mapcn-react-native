---
title: Location puck
description: MapLocationPuck replaces MapUserLocation (kept as a deprecated alias). Capability-gated props are documented, not faked.
---
...
```ts
import { MapLocationPuck, MapUserLocation, MAP_LOCATION_PUCK_CAPABILITIES } from "@/components/ui/map-location-puck";
```
...
### Deprecated: `MapUserLocation`

Kept as a v1-compatible alias that maps old prop names onto `MapLocationPuck`'s and prints a one-time `__DEV__` deprecation warning:

| v1 prop | Maps to |
| --- | --- |
| `showHeading` | `bearing: "heading"` |
| `showAccuracy` | `accuracyRing` |
| `animated` | `pulsing` (Mapbox) / no-op (MapLibre) |
